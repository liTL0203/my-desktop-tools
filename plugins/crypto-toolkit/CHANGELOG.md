...
## 新增
- 正则表达式测试台：匹配 / 提取分组 / 替换三模式，i/m/s/x 四旗标（RE2 语法，无灾难回溯），附语法速查
- Cron 表达式助手：解析与字段中文速读、未来 N 次执行时间预览，兼容 Linux 5 字段与 Quartz 6 字段，8 个常用模板一键填入

## 优化
- 编码 / 文本压缩 / 对称加密输出统一支持 TEXT·HEX·BASE64 三态本地切换，解密明文开箱可读
- 大输入前置拦截：超大数据毫秒级返回引导提示，不再等待超时
...
- **全部输出徽章放开 TEXT（UTF-8）态（用户反馈）**：盘点全部输出位——编码工具输出区（IOPanel，11 工具）、文本压缩输出（CompressPanel）与对称加密输出（上条）统一支持 TEXT/HEX/BASE64 三态本地切换：解码/解压出的明文直接可读，无需再手动转换；TEXT 按本地 UTF-8 解读输出字节（二进制内容显示替换符属预期，切回 HEX/BASE64）；⇄ 交换在 TEXT 态回填解码文本并自动置输入徽章为 TEXT；请求侧编码与展示态解耦（展示 TEXT 不影响 RPC 请求的 base64 输出）
- **对称加密输出 TEXT 态（用户反馈）**：SM4/AES 等对称加密的输出徽章放开 TEXT（UTF-8）模式——解密明文结果开箱可读，无需再手动转换；TEXT 本地按 UTF-8 解读输出字节（非文本密文显示替换符属预期，切回 HEX/BASE64 查看）；hex↔base64↔utf8 三态本地切换不重发请求
- **transform_run 大输入前置拦截（排查「RPC 超时 10s」根因）**：核心转发层单行响应上限 1MB（MAX_LINE_BYTES），大文本加密的 base64 输出超限被截断丢弃 → 前端只能干等超时且看不到任何错误——现在 sidecar 对输入 >512KB 直接毫秒级返回引导错误（「请改用文件加密/文件工具管线」），前端 ToolCanvas 同步预检不发请求；正常输入的错误（错密钥/错长度等）实测 25ms 内返回
- **正则表达式测试台（新分类「文本工具」）**：匹配（位置+内容+命名分组值）/ 提取分组（命名或编号）/ 替换（$1 与 ${name} 引用）三模式即算；i/m/s/x 四旗标；Rust regex（RE2 语法无灾难回溯，语法错误带位置）；帮助含语法速查（含 RE2 与 PCRE 的差异说明：不支持前后瞻与反向引用）
- **Cron 表达式助手**：解析 + 字段中文化速读 + 未来 N 次（3-20）执行时间预览（本地时区）；兼容 Linux 5 字段（自动补秒）/Quartz 6 字段，? 与 * 通配归一化互认；8 个常用模板一键填入（每分钟/每5分钟/工作日 9:30/每月 1 号等）；@daily 速写支持
