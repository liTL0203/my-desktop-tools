...
# v1.7.0 更新说明 (2026-09-23)

## 新增
- **粘贴自动提取 JSON**：粘贴的内容不是纯 JSON（日志前后缀、代码块包裹、多段 JSON 混杂）时，自动提取其中的 JSON 片段并格式化（取最长的合法片段），编辑区顶部显示蓝色提示条告知已提取
- **字典错误显式提示**：插件引擎（sidecar）版本过旧或写入失败时，字典管理抽屉顶部显示红色错误条——此前这类失败完全静默，重启后数据默默丢失

## 优化
- **格式化提速**：粘贴/打开文件的"解析→格式化→分析"合并为 Worker 单次往返（此前大 JSON 数据对象要跨线程克隆 3 次），大文档格式化显著变快
- **内置字典精简**：默认只保留一组「示例字典」（一条 status 字段）展示标注效果，业务字典请按需导入或自建；已有用户的字典数据不受影响

## 修复
- **字典导入选择文件后无反应**：选完文件现在自动进入向导第 2 步（此前停留在第 1 步看起来像没反应）；读取失败/空文件会显示明确错误提示
- **导入文件对话框看不到模板**：字典导入的文件选择框现在默认显示 CSV 文件类型（此前只有 JSON/所有文件，用户须手动切"所有文件"才能看到 .csv 模板）
- **导入向导重开状态残留**：关闭向导再打开会正确重置回第 1 步
- **导入 GBK/UTF-16 编码的 CSV 报错**：Excel 另存的 CSV（默认 ANSI/GBK 或 UTF-16 编码）此前读取报「stream did not contain valid UTF-8」——文件读取现按 BOM/编码自适应解码（UTF-8 · UTF-8 BOM · UTF-16 LE/BE · GBK，5 个单测覆盖）；导入模板下载改为带 UTF-8 BOM（Excel 直接打开中文不乱码，另存倾向保 UTF-8）

---

# v1.6.2 更新说明 (2026-09-21)

## 新增
- 字典命中卡片支持搜索过滤——枚举值较多时（超过 8 条）自动出现搜索框，快速定位目标枚举
- 卡片内滚动查看大量枚举值（限高防超出视口）；行贴近屏幕底部时卡片自动向上弹出，不再被裁切

---

# v1.6.1 更新说明 (2026-09-16)
...
# v1.6.0 更新说明 (2026-09-16)

## 新增
- 数据字典标注：JSON 字段命中字典时，树形视图自动显示字段说明与枚举翻译，悬停展示字典卡片；表格视图列头标注 + 单元格枚举翻译
- 字典管理：内置 API / 支付 / 用户三组通用字典，支持分组停用 / 编辑 / 删除与恢复默认，条目增删改即时生效
- 字典导入导出：三步导入向导（粘贴表格或 CSV/JSON 文件 → 列映射 → 字段绑定），支持导出 JSON 与团队共享

---

# v1.6.0 更新说明 (2026-09-20)

## 新增
- **数据字典标注**：JSON key 命中字典时，树形视图自动显示字段说明（IDE 式轻注解）与枚举值翻译（如 `status: 1 → 已支付`），悬停命中行显示字典卡片（说明/所属组/枚举表）——枚举多时可**在卡片内搜索过滤（>8 条出现搜索框）并滚动查看**，行贴近底部时卡片自动向上弹不被裁切；表格视图列头标注 + 单元格枚举翻译
- **字典管理**：右侧抽屉管理字典分组（内置三组：通用 API / 支付域 / 用户域，可停用/编辑/**删除**——删后可用「恢复默认字典」找回），条目增删改（字段名/说明/枚举键值对），保存后标注即时刷新，重启保留
- **字典导入向导**：三步导入——粘贴表格或选择文件（CSV/JSON）→ 列映射自动识别（DICT_TYPE/DICT_CODE/DICT_NAME 三列结构）→ 字段绑定（归一化匹配，多别名支持）；**提供导入模板下载**（CSV 模板照填即可导入）
- **字典导出**：导出全部或当前组为 JSON，与导入互通（round-trip 无损），可团队共享

---

# v1.5.0 更新说明 (2026-09-16)

## 优化
- 启动性能三期优化——多开与预建窗口协同更顺畅
- 覆盖窗口能力适配

---

# v1.4.0 更新说明 (2026-09-16)

## 新增
- **高级视图全选 / 复制按钮**：视图栏右侧新增两个按钮——「全选」把选区限定在当前视图数据区内（树形/表格/图片）；「复制」一键复制视图内容（已在视图内框选时仅复制所选；图片视图复制 URL 列表；表格复制所见文本），复制成功图标变 ✓
- **Ctrl+A 作用域修复**：结果视图区改为可聚焦容器——点进结果区后按 Ctrl+A 只全选视图内容，不再整页全选；焦点在搜索框等输入框内时保持原生行为

## 变更
- **恢复多开**：应用户决定 `allowMultiple` 改回 `true`——从快捷启动 / 快捷操作打开插件时每次新建独立窗口，可同开多个
- 取舍说明：多实例形态下核心自动跳过预建隐藏窗口提速（F147 设计上与多实例互斥），QA 直开回到常规温启动速度；粘贴/打开文件的即时格式化优化不受影响
