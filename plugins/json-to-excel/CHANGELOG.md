# v2.3.0 更新说明 (2026-09-23)

## 新增与修复
- 载入自动格式化：粘贴、拖入 .json、快捷操作传入的单行紧凑 JSON 自动展开为 2 空格缩进；已带换行的内容保持原排版不动
- 保存失败可见性：导出启动失败 / 轮询超时现在弹出红色提示（此前只显示在左栏底部小字，易被当成「没有反应」）
- 版本对齐：manifest(2.1.0)/package(2.2.1) 发版残留不一致，统一为 2.3.0
