...
- **Hardware Info**: CPU details (cores, frequency, temperature), memory, disk, GPU, motherboard/BIOS
- **Network Info**: IP/MAC/gateway/DNS/DHCP configuration, WiFi connection details (SSID/signal/channel/band/rate)
- **Real-time Monitoring**: Overview page with CPU/memory/disk usage ring charts, 30-second auto-refresh
...
- **硬件信息**：CPU 详情（核心数、频率、温度）、内存、磁盘、GPU、主板/BIOS
- **网络信息**：IP/MAC/网关/DNS/DHCP 配置、WiFi 连接详情（SSID/信号/信道/频段/速率）
- **实时监控**：概览页 CPU/内存/磁盘使用率环形图，30 秒自动刷新
- **进程列表**：按内存排序的 Top 10 进程
...
## 新增
- 初始版本，提供系统信息获取核心功能
- 支持查看 CPU、内存、磁盘、GPU、主板/BIOS 硬件信息
- 支持查看操作系统版本、主机名、运行时长、时区等系统信息
- 支持查看网络基础配置（IP/MAC/网关/DNS/DHCP）
- 支持查看当前 WiFi 连接详情（SSID/信号/信道/频段/速率）
- 支持查看进程 Top 10（按内存排序）
- 概览页实时监控（CPU/内存/磁盘使用率环形图，30 秒刷新）
- 支持网络连通性检测（网关可达/Internet 连通/公网 IP）
- 支持一键复制字段/卡片，导出 TXT/JSON 报告
- 支持暗色/亮色主题自动切换，中英文双语界面
